...
template<class T>
// (я не знала как ещё обозначать пустые элементы такие как
// нулевые при работе метода setsize  ведь при 
// инициализации элементом по умолчанию (T())
// всё равно у node вызывается конструктор с параметром
// и элемент определяется как true во всех случаях
// что мешало работе многих методов и поэтому в метод
// append добавила доп. параметр по умолчанию
// чтобы не пришлось делать перегрузку append-a
// для создания чистых элементов, которые добавляются в setsize)
void Array<T>::append(T value, bool full_P)
{
    // создание элемента с новым значением и определением full-а
